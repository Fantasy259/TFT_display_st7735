...
int32_t myComando( const uint8_t *cmd, size_t cmd_size, const uint8_t *param, size_t param_size)
{
    ESP_LOGI("ST7735", "comando-> cmd=0x%02X, size cmd =%d , param = 0x%02x, size Param %d", cmd ? cmd[0] : 0, (int) cmd_size, param ? param[0] : 0, (int)param_size);
    esp_err_t ret;
...
static void TestSenden(void *pvParameter)
{
    char daten[100]; 
    ...
    while (1)
    {
        ...
        gpio_set_level(TFT_DC, 1);
            ...
            if (ret != ESP_OK)
            {
                ESP_LOGI("TestTask" , " Abbruch wegen fehler");
                return ;
        } 
